...
Elm.Debug.make = function (_elm) {
   ...
   var formatRelative = function (d$) {
      return function () {
         ...
         var d = d$ / 1000;
         return _U.cmp(d,
         minute) < 0 ? A2($Basics._op["++"],
         "a ",
         A2($Basics._op["++"],
         $Basics.toString(A2($Basics._op["%"],
         $Basics.round(d),
         60)),
         " seconds ago")) : _U.cmp(d,
         hour) < 0 ? A2($Basics._op["++"],
         $Basics.toString($Basics.round(d / minute)),
         " minutes ago") : _U.cmp(d,
         day) < 0 ? A2($Basics._op["++"],
         $Basics.toString($Basics.round(d / hour)),
         " hours ago") : "TODO";
      }();
   };
   var diffTime = F2(function (timeStr,
   now) {
      return function () {
         var _v0 = $Date.fromString(timeStr);
         switch (_v0.ctor)
         {case "Ok": return function () {
                 var diff = now - $Date.toTime(_v0._0);
                 return $Maybe.Just(formatRelative(diff));
              }();}
   ...
   var noOut = $Signal.sampleOn($Signal.constant(false))($Signal.constant(""));
   var UpdateMaxMessages = function (a) {
      return {ctor: "UpdateMaxMessages"
             ,_0: a};
   };
   var Preference = {ctor: "Preference"};
   var NoOp = {ctor: "NoOp"};
   var updates = $Signal.channel(NoOp);
   var Click = {ctor: "Click"};
   var update = F2(function (s,m) {
      return function () {
         switch (s.ctor)
         {case "Auto": return A2(update,
              Click,
              _U.replace([["autoUpdate"
                          ,$Basics.not(m.autoUpdate)]],
              m));
            case "ClearOld":
            return m.autoUpdate ? _U.replace([["old"
                                              ,m.$new]
                                             ,["$new",_L.fromArray([])]],
              m) : _U.replace([["old"
                               ,_L.fromArray([])]
            ...
            return _U.replace([["$new"
              ...
              m);
            case "Preference":
            return _U.replace([["isPreference"
                               ,$Basics.not(m.isPreference)]],
              m);
            case "ReadAll":
            return m.autoUpdate ? _U.replace([["old"
                                              ,A2($List.take,
            ...
            switch (s._0.ctor)
              {case "Ok":
                 return m.autoUpdate ? _U.replace([["$new"
                                                   ,A2($List.take,
                                                   m.maxMessages,
                                                   A2($List._op["::"],
                                                   s._0._0,
              ...
              break;
            case "Timed":
            return _U.replace([["now"
                               ,$Maybe.Just(s._0)]],
              m);
            case "UpdateMaxMessages":
            return function () {
                 var _v7 = $String.toInt(s._0);
                 switch (_v7.ctor)
                 {case "Ok":
                    return _U.replace([["maxMessages"
                                       ,_v7._0]],
                      m);}
                 return m;
              }();}
   ...
   var Auto = {ctor: "Auto"};
   var ClearOld = {ctor: "ClearOld"};
   var ReadAll = {ctor: "ReadAll"};
   var Timed = function (a) {
   ...
   var viewLogo = function (m) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("text-center")]),
      _L.fromArray([A2($Html.span,
      _L.fromArray([$Html$Attributes.$class("fa fa-plug")
                   ,$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                         ,_0: "font-size"
                                                         ,_1: "20rem"}]))]),
      _L.fromArray([]))]));
   };
   var viewPreferences = function (m) {
      return m.isPreference ? A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([A2($Html.label,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("max messages: ")]))
                   ,A2($Html.input,
                   _L.fromArray([A3($Html$Events.on,
                                "input",
                                $Html$Events.targetValue,
                                function ($) {
                                   return $Signal.send(updates)(UpdateMaxMessages($));
                                })
                                ,$Html$Attributes.value($Basics.toString(m.maxMessages))]),
                   _L.fromArray([]))])) : A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([]));
   };
   var viewPreferenceButton = function (m) {
      return A2($Html.button,
      _L.fromArray([$Html$Events.onClick(A2($Signal.send,
                   updates,
                   Preference))
                   ,$Html$Attributes.$class(m.isPreference ? "btn btn-primary active" : "btn btn-default")]),
      _L.fromArray([A2($Html.span,
                   _L.fromArray([$Html$Attributes.$class("fa fa-cog")]),
                   _L.fromArray([]))
                   ,$Html.text(" preferences")]));
   };
   var viewRawBody = function (body) {
      return !_U.eq(body,
      "") ? A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([A2($Html.h3,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Raw body:")]))
                   ,A2($Html.pre,
                   _L.fromArray([$Html$Attributes.$class("pre-scrollable")]),
                   _L.fromArray([$Html.text(body)]))])) : A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([]));
   };
   var viewRelative = F2(function (now,
   time) {
      return $Html.text(A2($Maybe.withDefault,
      "a few seconds ago",
      A2($Maybe.andThen,
      now,
      diffTime(time))));
   });
   var viewFile = F2(function (id,
   x) {
      return function () {
         ...
         "image/",
         x.contentType) ? A2($Html.div,
         _L.fromArray([$Html$Attributes.$class("col-xs-12 col-md-6")]),
         _L.fromArray([A2($Html.h6,
                      _L.fromArray([]),
                      ...
                      _L.fromArray([$Html.text(x.name)]))]))])) : A2($Html.div,
         _L.fromArray([]),
         _L.fromArray([A2($Html.b,
                      ...
                      _L.fromArray([$Html$Attributes.href(link)]),
                      _L.fromArray([$Html.text(x.name)]))]))
                      ,$Html.text(x.contentType)]));
      }();
   });
   var viewFiles = F2(function (m,
   hs) {
      return A2($Html.div,
      _L.fromArray([]),
      $List.isEmpty(hs) ? _L.fromArray([]) : A2($List._op["::"],
      A2($Html.h3,
      _L.fromArray([]),
      _L.fromArray([$Html.text("Files")])),
      A2($List.map,
      viewFile(m.logId),
      hs)));
   });
   var viewHeaders = function (hs) {
      return function () {
         var go = function (x) {
            return A2($Html.div,
            _L.fromArray([]),
            _L.fromArray([A2($Html.b,
                         _L.fromArray([]),
                         _L.fromArray([$Html.text(x.field)
                                      ,$Html.text(": ")]))
                         ,$Html.text(x.value)]));
         };
         return A2($Html.div,
         ...
         A2($Html.h3,
         _L.fromArray([]),
         _L.fromArray([$Html.text("Headers")])),
         A2($List.map,go,hs)));
      }();
   };
   var viewQueryStrings = function (qs) {
      return function () {
         var go = function (x) {
            return A2($Html.div,
            _L.fromArray([]),
         ...
         A2($Html.h3,
         _L.fromArray([]),
         _L.fromArray([$Html.text("Params")])),
         A2($List.map,go,qs)));
      }();
   };
   var showLog = F3(function (m,
   isNew,
   x) {
      return A2($Html.div,
      _L.fromArray([]),
      ...
      _L.fromArray([A2($Html.h4,
                   ...
                   _L.fromArray([A2($Html.b,
                                _L.fromArray([]),
                                _L.fromArray([$Html.text($Basics.toString(x.logId))]))
                                ,$Html.text(" - ")
                                ,A2($Html.b,
                                _L.fromArray([]),
                                _L.fromArray([A2(viewRelative,
                                m.now,
                                x.createdAt)]))]))
                   ,A2($Html.h4,
                   _L.fromArray([]),
                   _L.fromArray([A2($Html.b,
                                _L.fromArray([]),
                                _L.fromArray([$Html.text(x.method)]))
                                ,$Html.text(" ")
                                ,$Html.text(x.rawPath)
                                ,isNew ? A2($Html.span,
                                _L.fromArray([$Html$Attributes.$class("label label-info")]),
                                _L.fromArray([$Html.text("New")])) : A2($Html.div,
                                _L.fromArray([]),
                                _L.fromArray([]))]))
                   ,viewRawBody(x.rawBody)
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("col-md-6 col-xs-12")]),
                   _L.fromArray([viewQueryStrings(x.queryStrings)]))
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("col-md-6 col-xs-12")]),
                   _L.fromArray([viewHeaders(x.headers)]))
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("col-md-12 col-xs-12")]),
                   _L.fromArray([A2(viewFiles,
                   x,
                   x.files)]))]))]));
   });
   var view = function (m) {
      ...
      _L.fromArray([A2($Html.div,
                   ...
                   _L.fromArray([A2($Html.br,
                                ...
                                ,A2($Html.h3,
                                _L.fromArray([]),
                                _L.fromArray([$Html.text("Debug on you!!!")]))
                                ,A2($Html.div,
                                _L.fromArray([$Html$Attributes.$class("btn-group")]),
                                _L.fromArray([A2($Html.button,
                                             _L.fromArray([$Html$Events.onClick(A2($Signal.send,
                                                          updates,
                                                          Click))
                                                          ,$Html$Attributes.$class(_U.cmp(m.newCount,
                                                          0) > 0 ? "btn btn-success" : "btn btn-default")]),
                                             _L.fromArray([$Html.text(_U.cmp(m.newCount,
                                             0) > 0 ? A2($Basics._op["++"],
                                             "show new ",
                                             $Basics.toString(m.newCount)) : "read all")]))
                                             ,A2($Html.button,
                                             _L.fromArray([$Html$Events.onClick(A2($Signal.send,
                                                          updates,
                                                          Auto))
                                                          ,$Html$Attributes.$class(m.autoUpdate ? "btn btn-primary active" : "btn btn-default")]),
                                             _L.fromArray([A2($Html.span,
                                                          _L.fromArray([$Html$Attributes.$class("glyphicon glyphicon-refresh")]),
                                                          _L.fromArray([]))
                                                          ,$Html.text(m.autoUpdate ? "Disable auto-update" : "Enable auto-update")]))
                                             ,viewPreferenceButton(m)
                                             ,A2($Html.a,
                                             _L.fromArray([$Html$Attributes.href("/tools")
                                                          ,$Html$Attributes.target("_blank")
                                                          ,$Html$Attributes.$class("btn btn-default")]),
                                             _L.fromArray([A2($Html.span,
                                                          _L.fromArray([$Html$Attributes.$class("glyphicon glyphicon-cutlery")]),
                                                          _L.fromArray([]))
                                                          ,$Html.text(" tools")]))
                                             ,A2($Html.button,
                                             _L.fromArray([$Html$Events.onClick(A2($Signal.send,
                                             ...
                                             _L.fromArray([$Html.text("clear old")]))]))]))
                   ,viewPreferences(m)
                   ,A2($Html.div,
   ...
   var File = F2(function (a,b) {
      return {_: {}
             ,contentType: b
             ,name: a};
   });
   var parseFile = A3($Json$Decode.object2,
   File,
   A2($Json$Decode._op[":="],
   "name",
   $Json$Decode.string),
   ...
   "contentType",
   $Json$Decode.string));
   var QueryString = F2(function (a,
   b) {
      return {_: {}
             ,field: a
             ,value: b};
   });
   var parseQuery = A3($Json$Decode.object2,
   QueryString,
   A2($Json$Decode._op[":="],
   "field",
   $Json$Decode.string),
   ...
   var parseLog = A9($Json$Decode.object8,
   Log,
   A2($Json$Decode._op[":="],
   "logId",
   $Json$Decode.$int),
   ...
   _elm.Debug.values = {_op: _op
                       ...
                       ,parseFile: parseFile
                       ,parseLog: parseLog
                       ,viewQueryStrings: viewQueryStrings
                       ,viewHeaders: viewHeaders
                       ,viewFile: viewFile
                       ,viewFiles: viewFiles
                       ,viewRelative: viewRelative
                       ,viewRawBody: viewRawBody
                       ,showLog: showLog
                       ,viewPreferenceButton: viewPreferenceButton
                       ...
                       ,UpdateMaxMessages: UpdateMaxMessages
                       ,updates: updates
                       ,noOut: noOut
                       ,server: server
                       ,wsurl: wsurl
